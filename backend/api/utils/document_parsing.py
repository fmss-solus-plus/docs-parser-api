...
def download_file(file_url: str):
    try:
        ...
        if response.status_code != 200:
            return Response({"message": f'{STATUS_MESSAGES["errors"]["FAILED_DOWNLOAD"]}'},
                     status=STATUS_CODES["errors"][400])
        
        content_type = response.headers.get('Content-Type', '')
        if 'pdf' not in content_type:
            return Response({"message": f'{STATUS_MESSAGES["errors"]["UNSUPPORTED_FILE_FORMAT"]}'},
        ...
        return Response(
            {"message": f"{str(e)}"},
            status=STATUS_CODES["errors"][500]
        )
